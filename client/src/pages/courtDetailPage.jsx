...
import { getCourt } from "../api/court.js";
...
export default function CourtDetailPage() {
  ...
  return (
    <div style={{ padding: 16 }}>
      <Link to="/">← Back</Link>
      <h1>{court.Name}</h1>
      <div>Price: {court.Price ? `${court.Price} VND / hour` : "N/A"}</div>
      <div>
        {court.Covered ? "Covered" : "Open"} • {court.IsIndoor ? "Indoor" : "Outdoor"}
      </div>
      ...
      {court.Group?.Latitude && court.Group?.Longitude && (
      ...
      <ul>
        {services.length ? (
          services.map((s) => (
            <li key={s.id}>
              {s.name} — {s.available ? "Available" : "Unavailable"}
              {s.price !== null ? ` — ${s.price} VND` : ""}
            </li>
          ))
        ) : (
